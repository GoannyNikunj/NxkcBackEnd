...
var { generate1Day , generateLast5WorkingDays , Get1M , get1Y , convertToISO , getNextWorkingDay } = require('../Function/Function');
...
class class1 {
  ...
  static SetShare = async (req, res) => {
    try {
      if (req.body.Name && req.body.Price && req.body.Time && req.body.Parents) {
        ...
        if (!ShareExist) {
          ...
          await YearData.push({time:nextWorkingDay,value:SinleDayData[SinleDayData.length - 1].value});
  ...
  static PurchaseCurrency = async (req, res) => {
    ...
    try {
      ...
      if (req.body.UpdatedAmount !== undefined  && req.body.Name && req.body.Time) {
  ...
  static PurchaseShare = async (req, res) => {
    try {
      ...
      if ( req.body.ShareId && req.body.Quantity && req.body.UserId && req.body.PurchaseType) {
